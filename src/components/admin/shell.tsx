...
export function AdminShell({ children, user }: { children: React.ReactNode; user: { name: string | null; role: UserRole } }) {
          ...
          {s.items.map((item) => (
            <Link key={item.href} href={item.href} aria-current={isActive(item.href) ? 'page' : undefined}
              className={cn('mb-0.5 flex items-center gap-2.5 rounded-md px-2.5 py-2 text-[13.5px] font-medium transition-colors',
                isActive(item.href) ? 'bg-[#d8f36a] text-[#16392d]' : 'text-slate-300 hover:bg-white/5 hover:text-white')}>
  ...
  return (
    <div className="admin-workspace flex min-h-screen bg-[#f3f5f4]">
      <aside className="admin-rail fixed inset-y-3 left-3 z-40 hidden w-56 flex-col rounded-2xl bg-[#162d28] lg:flex">
        <div className="flex h-16 items-center border-b border-white/10 px-4"><Logo light /></div>
        {nav}
        <div className="border-t border-white/10 p-3">
          <Link href="/" className="mb-1 flex items-center gap-2.5 rounded-md px-2.5 py-2 text-[13px] text-white hover:bg-white/5"><Store className="h-4 w-4" /> Ver a loja <ArrowUpRight className="ml-auto h-4 w-4" /></Link>
          <div className="mt-1 flex items-center justify-between px-2.5 py-2">
            ...
            <button onClick={async () => { await createClient().auth.signOut(); window.location.assign('/entrar') }} className="p-2 text-brand-200 hover:text-white" aria-label="Sair"><LogOut className="h-4 w-4" /></button>
          </div>
        </div>
      </aside>
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 flex w-64 flex-col bg-[#162d28]">
            <div className="flex h-16 items-center justify-between border-b border-white/10 px-4"><Logo light /><button onClick={() => setOpen(false)} className="p-1 text-white" aria-label="Fechar menu"><X className="h-5 w-5" /></button></div>
            {nav}
          </aside>
        </div>
      )}
      <div className="flex min-w-0 flex-1 flex-col lg:pl-60">
        <header className="admin-topbar sticky top-0 z-30 m-3 mb-0 flex min-h-16 items-center gap-3 rounded-xl bg-white px-4 py-3 lg:mx-6">
          ...
          <form action="/admin/produtos" role="search" className="flex min-w-0 flex-1 items-center gap-2 sm:max-w-sm">
            ...
            <input name="q" aria-label="Pesquisar produtos na gestão" placeholder="Pesquisar produto ou SKU…" className="h-9 w-full min-w-0 bg-transparent text-sm outline-none focus:ring-2 focus:ring-emerald-600" />
          </form>
          ...
          <details className="relative">
            <summary className="flex cursor-pointer list-none items-center gap-2">
              <span className="grid h-9 w-9 place-items-center rounded-full bg-[#eaf0d9] text-sm font-semibold text-[#27583e]">{(user.name ?? 'U').slice(0, 1).toUpperCase()}</span>
