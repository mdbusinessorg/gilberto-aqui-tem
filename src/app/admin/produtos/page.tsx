...
export default async function ProductsAdminPage({ searchParams }: { searchParams: Record<string, string | undefined> }) {
  ...
  let q = supabase.from('products').select('*, categories(name), brands(name)').order('created_at', { ascending: false }).limit(200)
        ...
        <Table>
          ...
          <TBody>
            {(products ?? []).map((p) => {
              ...
              return (
                <tr key={p.id}>
                  <td><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-md bg-surface text-ink-muted"><Package className="h-4 w-4" /></span><span className="font-medium">{p.name}{p.is_featured && <Badge tone="blue" className="ml-2">Destaque</Badge>}</span></div></td>
                  ...
                  <td><ProductRowActions product={p} categories={categories} brands={brands} /></td>
                </tr>
              )
            })}
          </TBody>
