...
export function HabitsChart({ data }: { data: { month: string; seen: number; sales: number }[] }) {
  if (!data.length) return <p className="py-10 text-center text-sm text-ink-muted">Sem dados</p>
  return (
    <div className="h-64">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} barGap={6} barCategoryGap="30%" margin={{ top: 8, right: 4, bottom: 0, left: -12 }}>
...
export function CategoryRings({ data, total }: { data: { category: string; units: number; revenue: number }[]; total: number }) {
  const top = data.slice(0, 4)
  const rings = top.map((d, i) => ({ name: d.category, value: d.units, fill: PALETTE[i % PALETTE.length] })).reverse()
  return (
    <div>
...
export function ChannelBubbles({ data }: { data: { label: string; value: number }[] }) {
    ...
    <div className="grid grid-cols-[1fr_auto] gap-4">
      <div className="relative h-40">
        {bubbles.map((b, i) => {
          ...
          return <span key={b.label} className="absolute grid place-items-center rounded-full text-xs font-semibold text-white" style={{ width: size, height: size, background: shades[i], ...positions[i] }}>{b.value}</span>
        })}
      ...
      <ul className="space-y-2 self-center text-xs">
        {bubbles.map((b, i) => <li key={b.label} className="flex items-center gap-2"><span className="h-3 w-3 rounded-sm" style={{ background: shades[i] }} />{b.label}</li>)}
      </ul>
    </div>
  )
}
